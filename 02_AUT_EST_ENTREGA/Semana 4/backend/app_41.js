...
app.post('/insereUsuario', urlencodedParser, (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*');
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	sql = "INSERT INTO usuario (nome, email, tel) VALUES ('" + req.body.nome + "', '" + req.body.email + "', " + req.body.telefone + ")";
	console.log(sql);
	db.run(sql, [], err => {
		if (err) {
			throw err;
...
app.post('/atualizaUsuario', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*');
	sql = "UPDATE usuario SET nome='" + req.body.nome + "', email = '" + req.body.email + "' , tel='" + req.body.tel + "' WHERE cod_usuario='" + req.body.cod_usuario + "'";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [], err => {
		n
